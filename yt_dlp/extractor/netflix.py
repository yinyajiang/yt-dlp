...
class NetflixIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        first = True
        for vcodec in self.VIDEO_PROFILES:
            first_acodec = next(iter(self.AUDIO_PROFILES.keys()))
            ...
            for subtitle_track in traverse_obj(manifest, ('result', 'timedtexttracks'), default=[]):
                ...
                downloadables = {k: downloadables[k] for k in self.SUBTITLE_EXT if k in downloadables}
                for fmt, data in downloadables.items():
