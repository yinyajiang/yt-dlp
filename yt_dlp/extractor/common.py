...
class InfoExtractor:
    ...
    def _correct_title_by_webview(self, title):
        if not title:
            return title
        match = re.search(r'Watch (.*?)(?:in .* )?on .*', title, re.IGNORECASE)
        if match:
            return match.group(1).strip()

        match = re.search(r'(.*?) Online Free .*', title, re.IGNORECASE)
        if match:
            title = match.group(1).strip()
            if title.startswith('Stream'):
                title = title[len('Stream'):].strip()
            return title
        return title
    ...
    def _extract_use_third_mutil_api(self, url, video_id=None):
        try:
            ...
            self.report_warning(f'_extract_use_third_mutil_api failed: {e}')
            return None
